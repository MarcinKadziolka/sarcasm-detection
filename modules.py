"""
Code for SelfAttention, SelfAttentionWide and TransformerBlock from:
...
class FastformerHead(nn.Module):
    ...
    def forward(self, x):
        ...
        query_att_logits = (
            self.to_query_att_logits(queries) / e**0.5
        )  # b, t, 1, because this simulates the logits wq^T q (skipping creating w)
        ...
        key_att_logits = (
            self.to_key_att_logits(p) / e**0.5
        )  # b, t, 1, because this simulates the logits wk^T p (skipping creating w)
...
class FastformerMultiHead(nn.Module):
    def __init__(self, emb, head_size, heads):
        ...
        self.heads = nn.ModuleList(
            [FastformerHead(emb, head_size) for _ in range(heads)]
        )
